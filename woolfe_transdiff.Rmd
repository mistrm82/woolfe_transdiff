...
ann <- cbind(id=rids, entrez=en, genename=gn, symbol=sm, ensembl=ens)
...
## Enrichment Analysis
For each of the three gene lists we can identify over-represented terms to get an idea of what pathways/processes are regulated. This was done using [GOstats](http://www.bioconductor.org/packages/release/bioc/html/GOstats.html), a Bioconductor package in R. The associated Excel spreadheets for each of the three gene lists are listed below:        

* MEFvDRG: [GO report](./results/GOStats/MEFvsDRG_GOresults.xls) /[KEGG report](./results/KEGG/MEFvsDRG_KEGGresults.xls)
* MEFvsiNoc: [GO report](./results/GOStats/MEFvsiNoc_GOresults.xls) /[KEGG report](./results/KEGG/MEFvsiNoc_KEGGresults.xls)
* iNocvsDRG: [GO report](./results/GOStats/iNocvsDRG_GOresults.xls) /[KEGG report](./results/KEGG/iNocvsDRG_KEGGresults.xls)
...
for (g in 1:3){
  ...
  #Get parameters
  GOparams <- new("GOHyperGParams", geneIds = unique(gsetEntrez), universeGeneIds = entrezUniverse, 
                annotation = "mogene10stprobeset", ontology = "BP", pvalueCutoff = 0.05, conditional = TRUE, 
              testDirection = "over")
  KEGGparams <- new("KEGGHyperGParams", geneIds = unique(gsetEntrez), universeGeneIds = entrezUniverse, 
                annotation = "mogene10stprobeset", pvalueCutoff = 0.05, testDirection = "over")
  
  # Run test
  hgOverGO <- hyperGTest(GOparams)
  hgOverKEGG <- hyperGTest(KEGGparams)
  
  # Write to file
  write.table(summary(hgOverGO), file=file.path(resultsDir, paste("GOStats/", genelists[g], "_GOresults.xls", sep="")),
              sep="\t", quote=F)
  write.table(summary(hgOverKEGG), file=file.path(resultsDir, paste("KEGG/", genelists[g], "_GOresults.xls", sep="")),
              sep="\t", quote=F)
...
```{r goresults, results='asis', echo=FALSE}
x <- read.delim(file.path(resultsDir, "GOStats/iNocvsDRG_GOresults.xls"), sep="\t", header=T, row.names=1)
x$Pvalue <- as.character(x$Pvalue)
kable(x[1:10,], format='markdown', row.names=F)
```


### KEGG
```{r keggresults, results='asis', echo=FALSE}
x <- read.delim(file.path(resultsDir, "KEGG/iNocvsDRG_KEGGresults.xls"), sep="\t", header=T, row.names=1)
x$Pvalue <- as.character(x$Pvalue)
kable(x[1:10,], format='markdown', row.names=F)
```

### Transcription factor enrichment
Since the process of transdifferentiation involves a cocktail of transcription factors, a logical next question that arises is whether there are specific TFs that bind to the regulatory regions in our list of DE genes from the iNocvsDRG comparison. We can check this by pulling a lis of TFs and their target genes from [oPPOSUM](http://burgundy.cmmt.ubc.ca/oPOSSUM/) and computing an overlap matrix with p-values generated using the hypergeometric distribution. 

```{r tfenrich, echo=FALSE}
